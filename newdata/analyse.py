...
def analyze_battle_complexity(model, dataloader, device):
    model.eval()
    
    total_complex_battles = 0
    total_battles = 0
    failed_complex_battles = 0
    failed_battles = 0
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            for i in range(len(batch['left'])):
                left_units = batch['left'][i]
                right_units = batch['right'][i]
                label = labels[i]
                pred = predictions[i]
                
                # 计算单位种数
                left_species = len(left_units) if isinstance(left_units, list) else 1
                right_species = len(right_units) if isinstance(right_units, list) else 1
                
                # 判断是否为大混战
                is_complex = (left_species + right_species >= 4)
                
                # 更新计数器
                total_battles += 1
                if is_complex:
                    total_complex_battles += 1
                
                # 检查预测是否失败
                if pred != label:
                    failed_battles += 1
                    if is_complex:
                        failed_complex_battles += 1
    
    return total_complex_battles, total_battles, failed_complex_battles, failed_battles
...
# 在整个测试集上进行分析
total_complex, total, failed_complex, failed = analyze_battle_complexity(model, test_dataloader, device)

# 计算比例
complex_ratio = total_complex / total * 100
failed_complex_ratio = failed_complex / failed * 100 if failed > 0 else 0

# 打印结果
print(f"数据集分析结果 (测试集):")
print(f"总战斗数量: {total}")
print(f"大混战数量: {total_complex} (比例: {complex_ratio:.2f}%)")
print(f"预测失败总数: {failed}")
print(f"预测失败中的大混战数量: {failed_complex} (比例: {failed_complex_ratio:.2f}%)")

# 可视化结果
labels = ['整体数据集', '预测失败案例']
complex_ratios = [complex_ratio, failed_complex_ratio]
