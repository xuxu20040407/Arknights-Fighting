...
class BattleModel(nn.Module):
    def __init__(self, feature_dims=[32,64], classifier_dims=[64,16], weight_dims=[4,4]):
        super().__init__()
        # 特征提取MLP (处理单位特征)
        input_dim = 9  # 单位ID(1) + 属性(8) [removed count]
        feature_layers = []
        prev_dim = input_dim
        ...
        # 权重MLP (处理单位ID和数量)
        self.id_embedding = nn.Embedding(1000, 8)  # Assuming max unit ID < 1000
        weight_layers = []
        prev_dim = 9  # embedded ID (8) + count (1)
        for dim in weight_dims:
            weight_layers.append(nn.Linear(prev_dim, dim))
            weight_layers.append(nn.ReLU())
            prev_dim = dim
        weight_layers.append(nn.Linear(prev_dim, 1))  # 输出bias
        self.weight_mlp = nn.Sequential(*weight_layers)
    ...
    def process_batch(self, batch):
        ...
        for units in batch:
            if units.numel() == 0:
                ...
                # 分离ID, count和其他属性
                unit_ids = units[:, 0].long()  # (num_units,) as integer IDs
                counts = units[:, 1].unsqueeze(-1)  # (num_units, 1)
                attributes = units[:, 2:]  # (num_units, 8)
                
                # 计算权重
                embedded_ids = self.id_embedding(unit_ids)  # (num_units, 8)
                weight_input = torch.cat([embedded_ids, counts], dim=1)  # (num_units, 9)
                bias = self.weight_mlp(weight_input).squeeze(-1)  # (num_units,)
                # weights = 0.1*counts.squeeze(-1) + bias  # weight = count + bias
                weights = bias
                weights = torch.sigmoid(weights)  # 限制权重在0-1范围
                
                # 拼接ID和其他属性 (去掉count)
                full_features = torch.cat([unit_ids.unsqueeze(-1).float(), attributes], dim=1)  # (num_units, 9)
                
                # 通过特征提取MLP
                unit_features = self.feature_mlp(full_features)  # (num_units, feature_dim)
                
                # 加权聚合特征
                weighted_features = unit_features * weights.unsqueeze(-1)
                agg_features = torch.sum(weighted_features, dim=0)
...
class BattleDataset(Dataset):
        ...
        for data in data_list:
            for side in ['left', 'right']:
                for unit_key, count in data[side].items():
                    ...
                    all_features.append(attrs)  # Only attributes for normalization
    ...
    def __getitem__(self, idx):
        ...
        for unit_key, count in battle['left'].items():
            ...
            attrs = self.monster_data[unit_id]
            # 只归一化属性
            normalized_attrs = self.scaler.transform(np.array(attrs).reshape(1, -1)).flatten()
            # 不归一化ID和count
            features = np.array([float(unit_id), float(count)] + list(normalized_attrs))
...
if __name__ == "__main__":
    ...
    # 初始化学习率调度器和保存路径
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.5)  # 每10个epoch学习率减半
